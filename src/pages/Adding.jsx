...
class _Adding extends Component {
    ...
    onUploadQuestion = (ev) => {
        ...
        setTimeout(async () => {
            if (!isInappropriate) {
                await this.props.addQuestion(this.state.final)
    ...
    render() {
        ...
        const sOrQ = currArray === tf ? 'statement' : 'question'
        return (

            <section className="adding pa">
                <header className="flex j-evenly">
                    <p onClick={() => this.setState({ isOptional: true, isTF: false, currArray: optional })} style={isOptional ? { color: 'black', fontWeight: 'bold', textShadow: '1px -1px 2px white' } : { color: 'white', fontWeight: 100 }}>Multiple Question</p>
                    <p onClick={() => this.setState({ isOptional: false, isTF: true, currArray: tf })} style={isTF ? { color: 'black', fontWeight: 'bold', textShadow: '1px -1px 2px white' } : { color: 'white', fontWeight: 100 }}>True/False Question</p>
                </header>
                <form >
                    ...
                    {currArray === tf && <div className="tf flex j-between">
                        ...
                        <p className="tf-desc pf ttc">In this type you are not supposed to ask a question, but to establish a statement whether it's correct or not enjoy!</p>
                    </div>}
                    ...
                    <div className="or tac ttc" style={{ fontSize: 'x-large', marginTop: '1%' }}>or</div>
                ...
                <div className={`finish-modal ${isFinishClicked ? 'open' : ''}`} style={window.screen.height < 1000 ? { height: currArray === tf ? '85vw' : '50vh' } : {}}>
                    ...
                    {isAllCompolete && !isLoadingFinal && !isDone && <div>
                        ...
                        <div className="submit flex a-center j-center pa" style={{ marginTop: currArray === tf ? '1%' : '5%' }} onClick={(ev) => this.onUploadQuestion(ev)}>
                            <p className="ma">Done</p>
                        </div>
                    </div>}
                    {isAllCompolete && isLoadingFinal && <div className="loading flex j-center">
                        <div className="lds-dual-ring flex a-center j-center"></div>
                        <small>Checking your {sOrQ}. Just a second...</small>
                    </div>}
                    {isDone && isInappropriate && <div className="inappropriate">
                        <p>Sorry, It seems your {sOrQ} is inappropriate...</p>
                        <div className="flex j-center a-center ma" onClick={() => this.setState({ isFinishClicked: false, isDone: false, isAllCompolete: false, isLoadingFinal: false, isInappropriate: false })}>Fix It</div>
                    </div>}
                    {isDone && !isInappropriate && <div className="done tac pr">
                        <span>Congratulation!</span> Your {sOrQ} added successfully
