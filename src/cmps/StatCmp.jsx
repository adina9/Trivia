...
class _StatCmp extends Component {
    ...
    openModal = (c, value) => {
        this.setState({ isModalOpen: true, currVal: value })
        this.getLevelsPoints(this.getCurrObj(c.n, value))
    }
    getCurrObj = (n, value) => this.props.pointsObj[value][n]
    getMaxForDispaly = () => this.state.currVal === "categories" ? 900 : 150
        ...
        return (
            <section className="stat-cmp pa" style={{ fontFamily: isEn ? 'montserrat' : 'sans-serif' }}>
                ...
                <section className="c-section flex column tac" style={{ opacity: isModalOpen ? 0 : 1 }}>
                    {finalStates?.map((s, idx) => <div className="arr-div" key={idx}>
                        <h3>{s.title}</h3>
                        <div className="c-div flex">
                            {s.arr.map((c, idx) => <div onClick={() => this.openModal(c, s.title)} className="c" key={idx}>
                                <img src={c.s} alt="" />
                ...
                <StatModal lang={this.props.lang} className={`stat-modal ${isModalOpen ? 'show' : ''}`} data={cData} options={options} closeModal={this.onCloseModal} />
            </section>
        );
    }
}
